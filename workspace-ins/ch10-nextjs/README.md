# 10장 Next.js
* 코드 실행(GitHub Page): <https://febc-15.github.io/react/workspace-ins/index.html#10>

## 목차
- [1. Next.js 개요](#1-nextjs-개요)
  - [1.1 React의 단점과 Next.js의 장점](#11-react의-단점과-nextjs의-장점)
  - [1.2 Next.js란?](#12-nextjs란)
  - [1.3 주요 특징](#13-주요-특징)
- [2. 개발환경 구성](#2-개발환경-구성)
  - [2.1 수동 구성](#21-수동-구성)
  - [2.2 자동 구성](#22-자동-구성)
...
* [웹 개발의 변천사](../ch01-buildup#1-웹-개발의-변천사)
...
* SEO(Search Engine Optimization, 검색 엔진 최적화) 어려움
  - 빈 HTML로 초기 응답하므로 검색 엔진이 읽을 컨텐츠가 없음
* 프론트엔드만 담당하므로 백엔드 API를 별도로 구축해야 함

### 1.1.2 Next.js의 장점
* 서버 사이드 렌더링(Server-Side Rendering, SSR)으로 초기 로딩 속도 개선
  - 완성된 HTML을 응답하므로 즉시 화면 표시 가능
* SEO 최적화
  - Next.js 서버에서 완성된 HTML을 응답하므로 검색 엔진이 컨텐츠를 바로 읽을 수 있음
* 파일 시스템 기반 자동 라우팅
  - pages나 app 폴더에 파일을 위치시키면 파일 시스템 기반의 라우팅 자동 생성
* 자동 코드 분할 및 성능 최적화
  - 페이지별 자동 번들 분할, 이미지/폰트 최적화 기능 내장
* 풀스택 개발 지원
  - API Routes로 백엔드 기능까지 하나의 프로젝트에서 개발 가능
* 다양한 렌더링 전략 지원
  - SSR, SSG(Static Site Generation, 사이트 정적 생성), ISR(Incremental Static Regeneration, 증분 정적 재생성) 선택 가능
...
* 개발 서버 구동 시 다음과 같은 에러가 발생할 경우 프로젝트 루트에 `next.config.js` 파일 생성
```sh
...
#### Next.js Router
- app 라우터: app 폴더 생성
- pages 라우터: pages 폴더 생성

## 2.2 자동 구성
### 2.2.1 create-next-app
* Next.js 프로젝트를 빠르게 생성하는 공식 CLI 도구
* 최신 Next.js 버전과 권장 설정으로 프로젝트 초기화
* TypeScript, ESLint, Tailwind CSS 등 옵션 선택 가능

#### 기본 사용법
```sh
# workspace/ch10-nextjs 폴더에서 실행
npx create-next-app@latest
```

#### 설정 옵션
* `create-next-app` 실행 시 다음 옵션들을 선택할 수 있음
  - 프로젝트 이름: 생성할 프로젝트 폴더 이름
  - 권장 기본값 사용: Yes 선택 시 Next.js 권장 설정 자동 적용
  - TypeScript: TypeScript 사용 여부
  - ESLint: ESLint 사용 여부
  - React Compiler: React Compiler 사용 여부
  - Tailwind CSS: Tailwind CSS 사용 여부
  - src/ 디렉토리: 코드를 `src/` 폴더 안에 배치할지 여부
  - App Router: App Router 사용 여부 (권장)
  - Import alias: 기본 `@/*` 별칭 사용 여부
...
# 3 프로젝트 구조
## 3.1 루트 폴더
<img src="../../images/nextjs/top-level-folders.png" width="800">
...
* `[[...folder]]`: Optional Catch-all 세그먼트
  ...
  - params 예시: `/docs` → `{}`, `/docs/a` → `{ slug: ['a'] }`

#### 라우트 그룹 및 프라이빗 폴더
* `(folder)`: 라우트 그룹
  - URL 경로에는 포함되지 않지만, 내부적으로 폴더를 그룹화하여 레이아웃/구조 관리
  - 예: `app/(user)/login/page.tsx` → 실제 경로는 `/login` (URL에 (`user`)가 포함되지 않음)
...
* `@folder`: 병렬 라우트(parallel routes)
  - 여러 UI 영역을 병렬로 렌더링할 때 사용
  - 예: `app/@modal/(...)` 등

* `(.)folder`, `(..)folder`, `(..)(..)folder`, `(...)folder`: 인터셉트 라우트(intercepting routes)
  - 특정 경로에서 다른 경로의 UI를 인터셉트(가로채기)하여 보여줄 때 사용
...
<img src="../../images/nextjs/terminology-component-tree.png" width="800">

* URL 경로(Path): 도메인 이후의 URL 부분
* URL 세그먼트(Segment): 슬래시로 구분된 URL 경로의 일부

<img src="../../images/nextjs/terminology-url-anatomy.png" width="800">
...
<img src="../../images/nextjs/route-segments-to-path-segments.png" width="800">

* 생성한 폴더 하위에 라우팅 규칙과 관련된 특수 파일, CSS, 이미지 등의 파일 생성
  - page.tsx: 해당 경로로 접근했을 때 보여줄 페이지 정의
  - page 파일이 없는 폴더는 라우팅 되지 않음

<img src="../../images/nextjs/defining-routes.png" width="800">
...
<img src="../../images/nextjs/page-special-file.png" width="800">
...
* page 컴포넌트를 children prop으로 받음
  - 레이아웃 파일에서 `{ children }` 으로 래핑해서 사용
* 하위 경로의 모든 layout은 중첩되어 사용됨
  - `children`은 현재 폴더부터 URL 경로와 일치하는 폴더까지 내려가면서 찾은 layout들이 중첩되고, 마지막에는 URL 경로에 존재하는 page 컴포넌트가 됨
* `layout.js`, `layout.jsx`, `layout.ts`, `layout.tsx`로 작성

#### 루트 레이아웃
* app 폴더 하위에 만들어야 하는 필수 layout 파일
* 모든 경로에 적용할 공통 UI 작성
* `<html>` `<body>` 태그를 필수로 작성해야 함
...
* layout, page에서 `metadata` 변수나 `generateMetadata` 함수를 내보내기 하면 메타데이터 정의 가능
  ...
  - generateMetadata: 동적인 메타데이터 출력

#### 메타데이터와 SEO
* 메타데이터는 주로 `<head>` 태그 내에 포함되는 title, description, keyword 등의 정보를 표현하는데 사용
* 메타데이터는 SEO(검색 엔진 최적화)에 중요한 역할을 하며, 소셜 미디어 공유 시에도 사용됨
* generateMetadata 함수 내에서 데이터를 fetching 하는 경우, Next.js는 이 데이터 fetching이 완료될 때까지 응답을 보내지 않음
* 데이터 fetching이 완료된 후 메타데이터를 최종적으로 생성하고, 이 메타데이터를 포함한 `<head>` 태그를 클라이언트로 스트리밍하기 시작
* 클라이언트는 서버로부터 받은 초기 컨텐츠가 `<head>`를 포함하고 있기 때문에 검색 엔진이 자바스크립트를 실행하지 않아도 완전한 메타데이터를 확인 할 수 있음
...
* App 라우터의 라우팅 기능을 구현한 컴포넌트

* `<a>` 태그 대신 사용
  - Link 컴포넌트는 렌더링 되면 `<a>` 로 바뀜
...
#### 주요 메서드
- `router.push(url)`: 지정한 경로로 이동 (브라우저 히스토리 스택에 추가)
- `router.replace(url)`: 지정한 경로로 이동 (현재 히스토리 항목을 교체, 히스토리 스택에 추가하지 않음)
...
* 기본적으로 307 상태 코드로 응답
  - 307 응답 상태코드: Temporary Redirect, 원래 요청 방식과 본문을 그대로 유지하면서 새로운 페이지 요청, 다음번 요청에도 이전 URL 사용
* `서버 함수`일 경우(POST 요청의 성공 페이지로 이동할 때) 303 응답 상태 코드로 응답
  - 303 응답 상태코드: See Other, 원래 요청 방식(POST)을 유지하지 않고 새로운 페이지로 GET 요청으로 변경하여 리디렉션
  - POST 요청 후 결과 페이지로 이동할 때 사용하여 폼 재제출 방지 (PRG 패턴: Post-Redirect-Get)
  - 307과의 차이: 307은 원래 요청 방식(POST)을 유지하지만, 303은 GET으로 변경
  
#### 예시
```tsx
'use server'; // 서버 함수(클라이언트에서 호출 할 수 있는 함수임을 표시)
import { redirect } from 'next/navigation';

export async function createPost(formData: FormData) {
  // 게시글 등록 로직...
  redirect('/posts'); // 등록 후 목록 화면으로 이동
}
```
...
## 4.6 동적 라우트
* 고정된 URL이 아닌 바뀔 수 있는 부분에 대해서 라우팅을 정의할 때 폴더명을 대괄호로 묶어서 생성
...
* app/posts/[id]/page.tsx에 추가
  ...
  export default async function InfoPage({ params }: { params: Promise<{ id: string }> }) {
    const { id } = await params; // params는 Promise이므로 await로 비동기 처리 필요
    return (
...
* Catch-all 세그먼트를 이용해서 특정 게시글에 달린 좋아요 목록, 관심글로 등록한 목록과 좋아요 상세정보, 관심글 상세 정보를 보여줄 때 만들어야 할 파일과 params 값
  - app/posts/[id]/[...slug]/page.tsx
    ...
    + /posts/3/likes/4 -> { id: '3', slug: ['likes', '4'] }
...
#### app/posts/[id]/page.tsx에 추가
  ...
  // 빌드하면 .next/server/app/posts/1.html, 2.html 생성
...
#### 개발 환경 vs 프로덕션 환경
* 개발 환경(`next dev`)
  - generateStaticParams()는 호출되지만 정적 페이지를 생성하지 않음
  - 모든 요청에 대해 동적으로 렌더링됨
  - 코드 변경 시 즉시 반영되어 개발 편의성 향상
* 프로덕션 빌드(`next build`)
  - 빌드 시점에 generateStaticParams()가 호출되어 정적 페이지 생성
  - 생성된 정적 페이지가 캐시되어 빠른 응답 제공

#### 빌드 시 동작 순서
1. 빌드 시 generateStaticParams() 함수 호출 후 반환 받은 배열의 각 요소를 params로 구성해서 Page 컴포넌트 호출
2. Page 컴포넌트에서 반환 받은 HTML을 빌드 결과로 저장(posts/1.html, 2.html)
3. 이후 브라우저의 posts/1 요청이 오면 빌드시 만들어둔 정적 라우팅 테이블에서 매칭되는 url이 있는지 확인 후 posts/[id]/page.tsx 파일을 실행하지 않고 posts/1.html을 응답
4. 브라우저가 posts/3 요청을 보내는 경우 정적 라우팅 테이블에 매칭되는 url이 없으므로 posts/[id]/page.tsx 파일을 실행하여 응답

## 4.7 라우트 그룹 및 프라이빗 폴더
### 4.7.1 라우트 그룹
* app 라우터는 app 하위 폴더가 URL 경로에 매핑됨
* 폴더가 URL 경로에 포함되지 않게 하고 싶을때 라우트 그룹을 생성
* `(user)` 처럼 폴더명에 `()`를 붙여서 작성
* URL에 영향을 주지 않고 여러 페이지를 하나의 폴더에 묶어서 관리
* 다른 라우트 그룹에 동일한 하위 경로 작성시 컴파일 에러 발생

<img src="../../images/nextjs/route-group-organisation.png" width="800">

#### user 폴더가 라우트 경로에 포함되는 경우
* login, signup 폴더를 user 폴더 하위에 생성한 경우 경로에 user가 포함됨
...
#### user 폴더를 제거하지 않고 경로에서만 제거하고 싶을 때
* 라우트 그룹으로 관리
...
<img src="../../images/nextjs/route-group-multiple-layouts.png" width="800">


#### account, cart, checkout 페이지에서 account, cart에만 동일한 layout을 적용하고 싶을 때

<img src="../../images/nextjs/route-group-opt-in-layouts.png" width="800">

#### 루트 레이아웃을 여러개 정의하고 싶을 때

<img src="../../images/nextjs/route-group-multiple-root-layouts.png" width="800">

### 4.7.2 프라이빗 폴더
* _로 시작하는 폴더는 page 파일이 있어도 라우팅에서 제외

<img src="../../images/nextjs/project-organization-private-folders.png" width="800">

#### 활용 사례
* `app` 폴더 내부에서만 사용되는 파일들을 `app` 폴더 내부에 정리하고 싶을 때
  - `app/_lib/`: `app` 폴더 내부의 여러 페이지에서만 사용되는 유틸리티 함수
  - `app/(admin)/_components/`: 특정 라우트 그룹(`(admin)`) 내에서만 사용되는 컴포넌트
...
### 4.8.2 Prefetching
* `<Link>` 컴포넌트를 사용하는 경우 링크를 누르기 전에 페이지를 미리 로드하는 작업
* 프로덕션 환경에서만 활성화 됨
...
<img src="../../images/nextjs/partial-rendering.png" width="800">

## 4.9 프로젝트 구성 및 라우트 관리
* 라우팅 폴더 내에 page, route 파일이 있는 경우에만 라우팅 됨

<img src="../../images/nextjs/project-organization-not-routable.png" width="800">

<img src="../../images/nextjs/project-organization-routable.png" width="800">
...
<img src="../../images/nextjs/project-organization-project-root.png" width="800">

#### 프로젝트 파일을 app 내부에 저장
* 모든 코드를 app 폴더 내부에 저장
* page, route 파일이 없으면 라우팅 되지 않음

<img src="../../images/nextjs/project-organization-app-root.png" width="800">

#### 기능이나 경로별로 파일 분할
* 공용 컴포넌트나 라이브러리를 app 폴더 하위에 작성하고 각 페이지별로 사용할 컴포넌트나 라이브러리는 각 페이지 폴더 하위에 작성

<img src="../../images/nextjs/project-organization-app-root-split.png" width="800">
...
* 내부적으로 React Suspense를 사용하여 컨텐츠가 로드되는 동안 대체할 컴포넌트로 사용됨
  - Next.js는 Page 컴포넌트가 async 함수일 때 반환되는 Promise를 이용해서 로딩중인지(pending) 완료되었는지(fulfilled, rejected) 상태를 추적해서 loading 컴포넌트를 보여줄 수 있음
...
* loading 파일을 만들어 두는 것 만으로도 loading 파일과 같은 폴더에 있는 layout 파일에 children을 `<Suspense>`로 감싼 것처럼 동작
  ```tsx
...
<img src="../../images/nextjs/loading-overview.png" width="800">
...
* 컴포넌트 렌더링시 에러가 발생할 경우 error 파일에서 에러 처리 및 에러 UI를 보여줌
  - 클라이언트 컴포넌트여야 함

### app/posts/error.tsx 작성
```tsx
'use client';
...
<img src="../../images/nextjs/error-overview.png" width="800">

<img src="../../images/nextjs/nested-error-component-hierarchy.png" width="800">
...
## 6.2 렌더링 방식
### 6.2.1 CSR vs. SSR
#### 6.2.1.1 CSR(Client Side Rendering)
* 리액트의 동작 방식
...
#### 6.2.1.2 SSR(Server Side Rendering)
...
* SSR을 지원하는 리액트 프레임워크로 Gatsby, Remix, Next.js 등이 있음
...
* FCP(First Contentful Paint)가 단축됨
  ...
  - HTML을 서버에서 만들어 주기 때문에 인터렉션에 필요한 자바스크립트를 실행하기 전이라도 화면에 보여줄 수 있음
  - 구글 등 검색 엔진의 페이지 순위 결정 요소 중 하나
* SEO에 유리
...
1. 라우트 세그먼트를 기준으로 각 세그먼트의 렌더링 작업 시작
  - /posts/3일 경우 /posts, /3 두 개의 세그먼트로 나뉨
...
### 7.3.2 `서버 액션`(Server Actions)
* `서버 함수` 중에서 form의 action prop으로 전달되거나 form의 action 핸들러 내부에서 호출되는 함수
...
### 7.3.7 `서버 함수` 호출
#### 7.3.7.1 form 요소의 action 속성으로 호출
...
### 7.4.2 컴포넌트 트리간 동일한 데이터 전달하지 않기
...
<img src="../../images/nextjs/server-rendering-with-streaming.png" width="800">

### 7.4.4 병렬 및 순차 fetch

<img src="../../images/nextjs/sequential-parallel-data-fetching.png" width="800">
